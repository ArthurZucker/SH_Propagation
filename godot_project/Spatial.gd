...
func handCard():
	for i in range(3):
		...
		if(card!=null):
			...
			mesh._setEndPosition(global.pos[global.controlGameNode.id_joueur]+Vector3(10,i,0))
			mesh.toBeMoved=1
			mesh.flip=0.5
...
func drawCard(v):
	...
	var mesh=cardNodes[card-1]
	mesh._setEndPosition(global.pos[global.controlGameNode.id_joueur]+Vector3(10,v,0))
	mesh.toBeMoved=1
	mesh.flip=0.5
