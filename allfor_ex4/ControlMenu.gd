...
func _thread_network_function(userdata):
	...
	if (socket.listen(4000,"192.168.1.75") != OK):
		print("An error occurred listening on port 4000")
		done = true;
	else:
		print("Listening on port 4000 on 192.168.1.75")
	while (done!=true):
...
func _on_ButtonJouer_pressed():
	...
	socket.set_dest_address("192.168.1.75", 4242)
...
func _on_ButtonTestReseau_pressed():
	print ("sending UDP test data to 192.168.1.75 port 4242")
	socket.set_dest_address("192.168.1.75", 4242)
